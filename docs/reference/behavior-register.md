...
| 7 | Durable delivery guarantee | W3C is silent on persistence across restarts; upstream has no persistent backend. | **Backend-dependent.** `SqliteStore` (single-node) is **exactly-once** for working memory — a crash mid-delivery rolls the event back and redelivers it, applied once (#21). `PostgresStore` (multi-node, `FOR UPDATE SKIP LOCKED` lease) is **at-least-once** — a crash after a side effect but before the row is deleted, or a lease that expires under a slow handler, redelivers the event, so **handlers must be idempotent** or externally deduplicated. Neither replays deterministically (only JSON-able WM + pending timers are persisted — the "Absurd" model, not Temporal's replay). | `durable.py` module docstring; `durable_postgres.py` module docstring | `test_durable.py` (exactly-once: `::..._crash_between_claim_and_persist_does_not_lose_event`); `test_durable_postgres.py` (at-least-once: `::test_pg_crash_between_claim_and_persist_redelivers`, `::test_pg_at_least_once_can_duplicate_side_effects`) |
